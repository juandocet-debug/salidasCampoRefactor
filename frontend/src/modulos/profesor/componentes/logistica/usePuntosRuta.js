...
import { MAX_HORAS_CONDUCTOR, calcularTiemposTramo, geocodificar } from './rutaUtils';
import { useRutaIA }       from './useRutaIA';
import { useAccionesRuta } from './useAccionesRuta';

// Re-exportar utilidades para que los consumidores existentes no rompan
export { parsearMinutos, fmtTiempo } from './rutaUtils';
...
export default function usePuntosRuta(form, setForm) {
    ...
    useEffect(() => {
        if (sincIda.current || !form._puntosRuta?.length >= 2) return;
        setPuntosRuta(form._puntosRuta);
        sincIda.current = true;
    }, [form._puntosRuta]);
    useEffect(() => {
        if (sincRetorno.current || !form._puntosRetorno?.length >= 2) return;
