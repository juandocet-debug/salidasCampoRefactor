...
const API_FAC  = `${API_URL}/api/admin/facultades/`;
const API_PROG = `${API_URL}/api/admin/programas/`;
...
export default function TabAcademico({ token, onToast }) {
    ...
    const cargar = useCallback(async () => {
        const [fRes, pRes] = await Promise.all([
            fetch(API_FAC,  { headers }).then(r => r.json()),
            fetch(API_PROG, { headers }).then(r => r.json()),
        ]);
        setFacultades(fRes.results || fRes);
        setProgramas(pRes.results  || pRes);
    }, [token]);

    useEffect(() => { cargar(); }, [cargar]);

    const crearFacultad  = async () => { if (!nuevaFac.nombre) return; await fetch(API_FAC, { method: 'POST', headers, body: JSON.stringify(nuevaFac) }); setNuevaFac({ nombre: '' }); agregarAlerta('Facultad creada correctamente', 'exito'); cargar(); };
    const crearPrograma  = async () => { if (!nuevoProg.nombre || !nuevoProg.facultad) return; await fetch(API_PROG, { method: 'POST', headers, body: JSON.stringify(nuevoProg) }); setNuevoProg({ nombre: '', facultad: '' }); agregarAlerta('Programa creado correctamente', 'exito'); cargar(); };

    const actualizarFacultad = async (id, datos) => { await fetch(`${API_FAC}${id}/`,  { method: 'PATCH', headers, body: JSON.stringify(datos) }); agregarAlerta('Facultad actualizada', 'exito'); cargar(); };
    const actualizarPrograma = async (id, datos) => { await fetch(`${API_PROG}${id}/`, { method: 'PATCH', headers, body: JSON.stringify(datos) }); agregarAlerta('Programa actualizado', 'exito'); cargar(); };

    const confirmarBorrado = async () => {
        if (!modalBorrar) return;
        setBorrando(true);
        const url = modalBorrar.tipo === 'facultad' ? API_FAC : API_PROG;
        await fetch(`${url}${modalBorrar.id}/`, { method: 'DELETE', headers });
