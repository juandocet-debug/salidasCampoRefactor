...
const CoordinadorDashboard = () => {
    ...
    const [metricas, setMetricas] = useState({ pendientes: 0, favorables: 0, rechazadas: 0 });
    ...
    useEffect(() => {
        ...
        .then(([pendientes, aprobadas]) => {
            ...
            const listAprob = Array.isArray(aprobadas) ? aprobadas : [];
            
            // Actualizar datos de la tabla según tab
            setSalidas(vistaActiva === 'pendientes' ? listPend : listAprob);
            
            // Calcular métricas
            const getEst = (s) => (s.estado || '').toLowerCase();
            const todas = [...listPend, ...listAprob];

            setMetricas({
                pendientes: listPend.length,
                favorables: todas.filter(s => ['favorable', 'favorable_con_ajustes', 'ajustada', 'aprobada', 'en_preparacion', 'en_ejecucion', 'finalizada', 'cerrada'].includes(getEst(s))).length,
                rechazadas: todas.filter(s => getEst(s) === 'rechazada').length,
            });
            setCargando(false);
        })
        .catch(err => {
        ...
        <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '20px' }}>
                ...
                <CardKPI 
                    ...
                    value={cargando ? '-' : metricas.favorables} 
                ...
                <CardKPI 
                    label="Rechazadas" 
                    value={cargando ? '-' : metricas.rechazadas} 
                    icon={<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>} 
                    iconBg="#fee2e2" 
                    iconColor="#dc2626" 
                />
            </div>
