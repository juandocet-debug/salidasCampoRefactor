...
const ListaTarjetasProfesor = ({ salidas = [], cargando = false, onSalidaEliminada }) => {
    ...
    const handleConfirmarBorrar = async () => {
        ...
        setBorrando(true);
        try {
            // Se delega la eliminación a la función inyectada (del hook useSalidas)
            if (onSalidaEliminada) await onSalidaEliminada(salidaABorrar.id);
            agregarAlerta(`Salida "${salidaABorrar.nombre}" eliminada.`, 'exito');
            setSalidaABorrar(null);
        } catch (error) {
            const msg = error.response?.data?.detail || error.message || 'No se pudo eliminar la salida.';
