...
import TabParametros from '../TabParametros/TabParametros';
...
export default function TabFlota({ token, onToast }) {
    ...
    const cargar = useCallback(async () => {
        ...
        try {
            ...
            const r = await clienteHttp.get(url);
            setVehiculos(r.data);
    ...
    const abrirCrear = () => {
        setEditando(null);
        setForm({ ...EMPTY_FORM }); // Changed from initialForm to EMPTY_FORM for syntactic correctness
        setFotoFile(null);
    ...
    const guardar = async (e) => {
        ...
        try {
            ...
            if (fotoFile instanceof File) {
                payload.append('foto_vehiculo', fotoFile);
            } else if (fotoRemovida) {
                payload.append('foto_vehiculo', '');
            }

            if (editando) {
                await clienteHttp.put(`${API}${editando.id}/`, payload);
                onToast('✅ Vehículo actualizado');
            } else {
                await clienteHttp.post(API, payload);
                onToast('✅ Vehículo registrado');
            }
            setModalOpen(false);
            cargar();
        } catch(e) {
            console.error("📛 Error HTTP desde Django:", e.response?.data || e);
            let msgBox = '❌ Error guardando vehículo';
            if (e.response && e.response.status === 400 && e.response.data) {
                const ers = Object.entries(e.response.data)
                    .map(([key, val]) => `${key.toUpperCase()}: ${Array.isArray(val) ? val.join(' ') : val}`)
                    .join(' | ');
                msgBox = `⚠️ Revisa el formulario: ${ers}`;
            }
            onToast(msgBox);
        }
        setGuardando(false);
    ...
    const eliminar = async (id) => {
        ...
        try {
            ...
            onToast('❌ Error eliminando vehículo');
        }
    };

    // ── IA: Chatbot Asistente ────────────────────────────────────────────────
    const consultarIA = async () => {
        ...
        setIaNota('');
        try {
            const r = await clienteHttp.post(API_IA, { consulta });
            const data = r.data;
            if (data.ok && data.respuesta) {
                setIaNota(data.respuesta);
                onToast(`🤖 Asistente IA ha respondido.`);
            } else {
                setIaNota('⚠️ No se obtuvo respuesta del Asistente.');
            }
        } catch { 
            setIaNota('❌ IA no disponible o falló la conexión'); 
            onToast('❌ Error de red consultando IA');
        }
        setIaLoading(false);
    };
                                ...
                                <div className="flota-col-left">
                                    ...
                                    <div className="flota-form-section" style={{ margin: 0, paddingRight: '12px' }}>
                                        ...
                                        <div className="flota-field" style={{ marginBottom: '16px' }}>
                                            ...
                                            <label className="flota-file-wrapper" style={{ minHeight: '120px', padding: (fotoFile || editando?.foto_url) ? '8px' : '24px 16px' }} htmlFor="foto_input">
                                                <input type="file" id="foto_input" accept="image/*" onChange={e => { setFotoFile(e.target.files[0]); setFotoRemovida(false); }} className="flota-file-input" />
                                                
                                                {!fotoFile && (!editando || !editando.foto_url) && (
                                                ...
                                                {editando?.foto_url && !fotoFile && (
                                                    <img src={editando.foto_url?.startsWith('http') ? editando.foto_url : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}${editando.foto_url}`} alt="Actual" className="flota-preview-img" style={{ width: '100%', height: '180px', objectFit: 'cover', borderRadius: '8px' }} />
                                                )}
                                            </label>
                                            {(fotoFile || editando?.foto_url) && (
                                                <button 
                                                    className="flota-btn-remove-photo" 
                                                    onClick={() => { setFotoFile(null); setFotoRemovida(true); }}
                                                    title="Remover foto"
                                                >
                                                    Remover foto
                                                </button>
                                ...
                                <div className="flota-col-right">
                                    ...
                                    <div className="flota-form-section">
                                        ...
                                        <div className="flota-form-grid">
                                            <div className="flota-field">
                                                <label>Año</label>
                                                <input type="number" min="1980" max="2050" value={form.anio} onChange={e => setForm(f => ({...f, anio: parseInt(e.target.value) || ''}))} placeholder="Ej: 2018" />
                                            </div>
                                            <div className="flota-field">
                                                <label>Cap./PAX</label>
